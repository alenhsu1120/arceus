---
name: rpg-convert
description: RPG→Python 全流程轉換 — rpg-analyzer → planner → coder → researcher → tester → api-writer → reviewer 依序執行，遇到失敗或不確定情況會停下來詢問使用者
triggers: ["rpg-convert", "RPG轉換", "RPG全流程轉換"]
agents: [rpg-analyzer, planner, coder, researcher, tester, api-writer, reviewer]
verification: [build, test]
---

# RPG Convert Workflow

你正在執行 **RPG Convert 全流程轉換模式**。針對使用者指定的 Job（例如 `EE18/Job15`），依序委派 7 個 agent 完成「分析 → 規劃 → 實作 → 文件 → 測試 → API → 審查」，每個階段都要看到明確結果才進入下一階段。
...
可一次接收多個 Job；逐個依序完整跑完七階段再換下一個 Job（不要並行跑多個 Job，避免同時大量寫入互相干擾）。
...
- coder 報告中列為「假設」的項目，一併記錄，留待 Step 7 review 時再確認一次。
...
## Step 7：reviewer（delegate to arceus:reviewer）

- 審查本次新增/修改的全部檔案（`dal.py`、業務邏輯檔、`README.md`、測試檔、endpoint 檔）。
- 若 verdict 是 `REQUEST_CHANGES` 或有 `[BLOCK]` 項目：交回對應 agent（coder 或 api-writer）修正 → 重新驗證（回到 Step 5 / Step 6）→ 再次 review。
